...
TEST_CASE("T", "[template]") {
  REQUIRE(
      falg::EachMul(std::array<float, 2>{1, 2}, std::array<float, 2>{1, 2}) ==
      std::array<float, 2>{1, 4});
  REQUIRE(falg::Dot(std::array<float, 3>{1, 2, 3},
                    std::array<float, 3>{1, 2, 3}) == 14);
  REQUIRE(falg::Dot(DirectX::XMFLOAT3{1, 2, 3}, DirectX::XMFLOAT3{1, 2, 3}) ==
          14);
  REQUIRE(falg::Dot(std::array<float, 4>{1, 2, 3, 4},
                    std::array<float, 4>{1, 2, 3, 4}) == 30);
...
TEST_CASE("Transform", "[order]") {
  auto a = falg::Transform{
      {1, 0, 0},
      falg::QuaternionAxisAngle({1, 0, 0}, 90.0f * falg::TO_RADIANS)};
  auto aa = a.ApplyPosition({1, 2, 3});
  REQUIRE(falg::Nearly(aa, std::array<float, 3>{2, -3, 2}));

  auto b = falg::Transform{
      {1, 0, 0},
      falg::QuaternionAxisAngle({0, 1, 0}, 90.0f * falg::TO_RADIANS)};
