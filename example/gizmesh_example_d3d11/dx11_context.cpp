...
public:
  ...
  ID3D11Device *Create(HWND hWnd) {
    ...
    if (FAILED(D3D11CreateDeviceAndSwapChain(
      ...
      std::cerr << "Error initializing DirectX" << std::endl;
      return nullptr;
    }
  ...
  ID3D11DeviceContext *NewFrame(int width, int height) {
    ...
    if (!m_rt.m_rtv) {
      ...
      if (FAILED(m_swapchain->GetBuffer(0, IID_PPV_ARGS(&backBuffer)))) {
        // fatal
        std::cerr << "fail to get backbuffer" << std::endl;
        return nullptr;
      }
  ...
  void ClearDepth() { m_rt.ClearDepth(m_context.Get(), 1.0f, 0xff); }

  void Present() { m_swapchain->Present(1, 0); }
};

//////////////////////////////////////////////////////////////////////////////
DX11Context::DX11Context() : m_impl(new DX11ContextImpl) {}

DX11Context::~DX11Context() { delete m_impl; }

void *DX11Context::Create(void *hwnd) { return m_impl->Create((HWND)hwnd); }

void *DX11Context::NewFrame(int width, int height) {
  return m_impl->NewFrame(width, height);
}

void DX11Context::ClearDepth() { m_impl->ClearDepth(); }

void DX11Context::Present() { m_impl->Present(); }

void *DX11Context::GetDeviceContext() { return m_impl->GetDeviceContext(); }
