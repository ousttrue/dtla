...
struct Dx11RenderTarget {
  ...
  bool Initialize(ID3D11Device *device,
    ...
    // create RTV
    if (FAILED(
            device->CreateRenderTargetView(texture.Get(), nullptr, &m_rtv))) {
      return false;
    }
    ...
    // create dsv
    if (FAILED(device->CreateDepthStencilView(depthBuffer.Get(), nullptr,
                                              &m_dsv))) {
      return false;
    }
  ...
  void ClearAndSet(ID3D11DeviceContext *context, const float *clear,
                   float depth, UINT8 stencil, int width, int height) {
    if (m_rtv) {
      context->ClearRenderTargetView(m_rtv.Get(), clear);
    }
    if (m_dsv) {
      context->ClearDepthStencilView(m_dsv.Get(), D3D11_CLEAR_DEPTH, depth,
                                     stencil);
    }
