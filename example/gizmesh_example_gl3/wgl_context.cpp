...
public:
  ...
  bool Initialize(HWND hwnd, int major, int minor) {
    ...
    static PFNWGLCREATECONTEXTATTRIBSARBPROC wglCreateContextAttribs;
    {
      ...
      if (platformVMaj < major ||
          (platformVMaj >= major && platformVMin < minor)) {
        fprintf(stderr,
                "OpenGL version %d.%d is not available (available version is "
                "%d.%d).",
                major, minor, platformVMaj, platformVMin);
        fprintf(stderr,
                "This error may occur if the platform has an integrated GPU.");
        return false;
      }
    ...
    wglSwapIntervalEXT(
        0); // example uses FPS as a rough perf measure, hence disable vsync
    ...
    if (major == 3 && minor == 1) {
      ...
      if (maxUniformBlockSize < (64 * 1024)) {
        fprintf(stderr, "GL_MAX_UNIFORM_BLOCK_SIZE is less than 64kb (%dkb)",
                maxUniformBlockSize / 1024);
        return false;
      }
    }
...
WGLContext::WGLContext() : m_impl(new WGLContextImpl) {}

WGLContext::~WGLContext() { delete m_impl; }
