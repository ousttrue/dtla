...
static void APIENTRY gl_debug_callback(GLenum source, GLenum type, GLuint id,
  ...
  std::cout << "gl_debug_callback: " << sourceStr << ", " << severityStr << ", "
            << typeStr << " , " << id << ", " << message << std::endl;
...
void GlTexture2D::setup(GLsizei width, GLsizei height, GLenum internal_fmt,
                        GLenum format, GLenum type, const GLvoid *pixels,
                        bool createMipmap) {
  glTextureImage2DEXT(*this, GL_TEXTURE_2D, 0, internal_fmt, width, height, 0,
                      format, type, pixels);
  if (createMipmap)
    glGenerateTextureMipmapEXT(*this, GL_TEXTURE_2D);
  glTextureParameteriEXT(*this, GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                         GL_LINEAR);
  glTextureParameteriEXT(*this, GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                         createMipmap ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
  glTextureParameteriEXT(*this, GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                         GL_CLAMP_TO_EDGE);
  glTextureParameteriEXT(*this, GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                         GL_CLAMP_TO_EDGE);
  this->width = static_cast<float>(width);
  this->height = static_cast<float>(height);
}

void GlTexture3D::setup(GLenum target, GLsizei width, GLsizei height,
                        GLsizei depth, GLenum internal_fmt, GLenum format,
                        GLenum type, const GLvoid *pixels) {
  glTextureImage3DEXT(*this, target, 0, internal_fmt, width, height, depth, 0,
                      format, type, pixels);
    ...
    if (indexCount) {
      ...
      if (instances)
        glDrawArraysInstanced(
            drawMode, 0, static_cast<GLsizei>(vertexBuffer.size / vertexStride),
            instances);
      else
...
void GlMesh::set_index_data(GLenum mode, GLenum type, GLsizei count,
