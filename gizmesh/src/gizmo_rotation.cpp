...
static bool dragger(const GizmoComponent &component, const falg::Ray &worldRay,
                    const GizmoState &state, falg::Transform *out,
                    bool is_local) {
  auto start_orientation =
      is_local ? state.original.rotation : falg::float4{0, 0, 0, 1};
  falg::Transform original_pose = {state.original.translation,
                                   start_orientation};
  auto the_axis = original_pose.ApplyDirection(component.axis);
  auto t = worldRay >>
           falg::Plane{the_axis, state.original.translation + state.offset};
  if (t < 0) {
    return false;
  }

  auto center_of_rotation =
      state.original.translation + the_axis * falg::Dot(the_axis, state.offset);
  auto arm1 = falg::Normalize(state.original.translation + state.offset -
                              center_of_rotation);
  ...
  out->rotation = falg::QuaternionMul(start_orientation,
                                      falg::QuaternionAxisAngle(a, angle));
  return true;
}

static falg::float2 ring_points[] = {
...
static GizmoComponent componentX{
    geometry_mesh::make_lathed_geometry({1, 0, 0}, {0, 1, 0}, {0, 0, 1}, 32,
                                        ring_points, _countof(ring_points),
                                        0.003f),
    {1, 0.5f, 0.5f, 1.f},
    {1, 0, 0, 1.f},
    {1, 0, 0},
...
static void draw_global_active(std::vector<gizmo_renderable> &drawlist,
                               const falg::Transform &gizmoTransform,
                               const GizmoComponent *active,
                               const GizmoState &state) {
  // auto modelMatrix =
  // falg::size_cast<minalg::float4x4>(gizmoTransform.matrix());
...

  {
    ...
    auto geo = geometry_mesh::make_lathed_geometry(
        ...
        falg::size_cast<falg::float3>(zDir), 32, arrow_points,
        _countof(arrow_points));
...
static void draw(std::vector<gizmo_renderable> &drawlist,
                 const falg::Transform &gizmoTransform,
                 const GizmoComponent *active) {
...
bool rotation(const GizmoSystem &ctx, uint32_t id, bool is_local,
  ...
  auto gizmoTransform =
      falg::Transform{t, r}; // Orientation is local by default
  if (parent) {
  ...
  // raycast
  {
    ...
    if (impl->state.has_clicked) {
      if (mesh) {
        ...
        auto worldOffset =
            gizmoTransform.ApplyPosition(localHit) - world.translation;
        gizmo->begin(mesh, worldOffset,
                     {world.translation, world.rotation, {1, 1, 1}}, {});
      }
  ...
  if (active) {
    ...
    if (!is_local) {
      r = falg::QuaternionMul(gizmo->m_state.original.rotation,
