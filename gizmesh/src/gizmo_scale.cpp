...
static bool dragger(const GizmoComponent &component, const falg::Ray &worldRay,
                    const GizmoState &state, bool uniform, falg::float3 *out) {
  auto plane_tangent =
      falg::Cross(component.axis, state.original.translation - worldRay.origin);
  auto N = falg::Cross(component.axis, plane_tangent);

  // If an intersection exists between the ray and the plane, place the object
  // at that point
  auto t =
      worldRay >> falg::Plane{N, state.original.translation + state.offset};
  if (t < 0) {
    return false;
  }
  auto intersect = worldRay.SetT(t);
  auto offset_on_axis = falg::EachMul(
      (intersect - state.offset - state.original.translation), component.axis);
  flush_to_zero(offset_on_axis);
  auto new_scale =
      state.original.scale + falg::size_cast<falg::float3>(offset_on_axis);

  if (uniform) {
    auto s = clamp(falg::Dot(intersect, new_scale), 0.01f, 1000.f);
    *out = falg::float3{s, s, s};
  } else {
    *out = falg::float3{clamp(new_scale[0], 0.01f, 1000.f),
...
static const GizmoComponent *g_meshes[] = {&xComponent, &yComponent,
                                           &zComponent};
...
static void draw(const falg::Transform &t,
